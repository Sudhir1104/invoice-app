...
export default function Onboarding({ user, onComplete }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await supabase.from("profiles").insert({ tenant_id: tenant.id, company_name: coName, abn: coAbn, address: coAddr, phone: coPhone, email: user.email });
      await supabase.from("counters").insert({ tenant_id: tenant.id, invoice_count: 0, quote_count: 0 });

      const { data: existing } = await supabase.from("user_settings").select("id").eq("user_id", user.id).maybeSingle();
      if (existing) {
        await supabase.from("user_settings").update({ tenant_id: tenant.id, role: "owner", email: user.email, onboarded: true }).eq("user_id", user.id);
      } else {
        await supabase.from("user_settings").insert({ user_id: user.id, tenant_id: tenant.id, role: "owner", email: user.email, onboarded: true });
      }
  ...
  const ink = NAVY;
  return (
    <div style={{ minHeight: "100vh", background: "#E8E4D0", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 20, backgroundImage: "repeating-linear-gradient(45deg, transparent, transparent 2px, rgba(160,150,100,0.04) 2px, rgba(160,150,100,0.04) 4px)" }}>
      ...
      <div style={{ background: "#FEFCE8", border: "1px solid #C8C0A0", borderLeft: "6px solid #B8A870", boxShadow: "0 8px 40px rgba(0,0,0,0.15)", padding: "36px 36px 32px", width: "100%", maxWidth: 460 }}>
        ...
        <button onClick={handleSubmit} disabled={loading || !coName || !coAbn}
          ...
          {loading ? "Setting up your workspace..." : "Start Invoicing →"}
        </button>
