...
export default function Admin({ user, onBack }) {
  ...
  const deleteAllDocuments = async () => {
    if (deleteAllConfirm.toLowerCase() !== "delete all") {
      showToast("❌ Type 'delete all' to confirm");
      return;
    }
    setToggling(selectedTenant.id);
    try {
      const { error } = await supabase
        .from("documents")
        .delete()
      ...
      if (error) throw error;
      setTenantDocs([]);
      setUsers(u => u.map(t => t.tenant_id === selectedTenant.id ? { ...t, docCount: 0 } : t));
      setShowDeleteAll(false);
      setDeleteAllConfirm("");
      showToast("✓ All documents deleted for " + selectedTenant.name);
    } catch (e) {
            ...
            <div style={{ background: "#fff", borderRadius: 12, padding: 24, maxWidth: 700, width: "100%", maxHeight: "80vh", overflow: "auto", boxShadow: "0 8px 40px rgba(0,0,0,0.3)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {tenantDocs.length > 0 && (
                    <button onClick={() => { setShowDeleteAll(true); setDeleteAllConfirm(""); }}
                      style={{ padding: "6px 14px", borderRadius: 6, border: "1.5px solid " + RED, background: "#FEE2E2", color: RED, fontFamily: "monospace", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                      🗑 Delete All ({tenantDocs.length})
                    </button>
                  )}
                  <button onClick={() => { setSelectedTenant(null); setShowDeleteAll(false); setDeleteAllConfirm(""); }}
                    style={{ padding: "6px 14px", borderRadius: 6, border: "1px solid #ddd", background: "#f5f5f5", fontFamily: "monospace", fontSize: 12, cursor: "pointer" }}>
                    Close
                  </button>
                </div>
              </div>

              {/* Delete All confirmation panel */}
              {showDeleteAll && (
                <div style={{ background: "#FEE2E2", border: "1.5px solid " + RED, borderRadius: 8, padding: 16, marginBottom: 16 }}>
                  <div style={{ fontFamily: "monospace", fontSize: 13, fontWeight: 700, color: RED, marginBottom: 6 }}>
                    ⚠️ Delete ALL {tenantDocs.length} documents for {selectedTenant.name}?
                  </div>
                  <div style={{ fontFamily: "monospace", fontSize: 11, color: "#7f1d1d", marginBottom: 10 }}>
                    This is permanent and cannot be undone. Type <strong>delete all</strong> to confirm.
                  </div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <input
                      ...
                      style={{ flex: 1, padding: "7px 12px", border: "1.5px solid " + RED, borderRadius: 6, fontFamily: "monospace", fontSize: 12, outline: "none" }}
                    />
                    <button
                      onClick={deleteAllDocuments}
                      disabled={deleteAllConfirm.toLowerCase() !== "delete all" || toggling === selectedTenant.id}
                      style={{ padding: "7px 16px", borderRadius: 6, border: "none", background: deleteAllConfirm.toLowerCase() === "delete all" ? RED : "#ccc", color: "#fff", fontFamily: "monospace", fontSize: 12, fontWeight: 700, cursor: deleteAllConfirm.toLowerCase() === "delete all" ? "pointer" : "not-allowed" }}>
                      {toggling === selectedTenant.id ? "Deleting..." : "Confirm Delete All"}
                    </button>
                    <button onClick={() => { setShowDeleteAll(false); setDeleteAllConfirm(""); }}
                      style={{ padding: "7px 14px", borderRadius: 6, border: "1px solid #ddd", background: "#fff", fontFamily: "monospace", fontSize: 12, cursor: "pointer" }}>
                      Cancel
                    </button>
                  </div>
                </div>
