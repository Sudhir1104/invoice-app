...
5.1 To the maximum extent permitted by law, the Seller's liability is limited to the value of the invoice.
...
// ── T&C + logo stay in localStorage (device preferences) ──
function loadTC() {
  try { return localStorage.getItem(TC_KEY) || DEFAULT_TC; } catch { return DEFAULT_TC; }
}
function saveTC(text) {
  try { localStorage.setItem(TC_KEY, text); } catch {}
}
...
// ══════════════════════════════════════════════
// SUPABASE DATA LAYER — matched to actual schema
// ══════════════════════════════════════════════
...
async function dbSaveDocument(doc, payStatus, docId) {
  ...
  if (docId) {
    const { data, error } = await supabase
      ...
      .select()
      .single();
    if (error) { console.error("Update document error:", error); return null; }
    return data;
  } else {
    ...
    const { data, error } = await supabase
      ...
      .select()
      .single();
    if (error) { console.error("Insert document error:", error); return null; }
    return data;
  }
}
...
// ── Counters — uses invoice_count / quote_count columns ──
...
// ── Profiles — uses company_name, abn, address, phone columns ──
...
async function dbLoadProfile() {
  ...
  if (error || !data) return loadProfile(); // fallback to localStorage
  ...
  // Merge logo from localStorage cache
...
async function dbSaveProfile(profileData) {
  const userId = await getUserId();
  if (!userId) return;
  saveProfile(profileData); // always cache locally
...
// ── Shape converter: Supabase row → app doc ──
...
function calcRow(row) {
  ...
  const each = parseFloat(row.each) || 0;
  const sub = qty * each;
...
function calcTotals(rows) {
  let sub = 0;
  rows.forEach(r => { sub += (parseFloat(r.qty) || 0) * (parseFloat(r.each) || 0); });
  const gst = Math.round(sub * GST_RATE * 100) / 100;
...
function AutoTextarea({ value, onChange, placeholder, style }) {
  ...
  return (
    <textarea ref={ref} value={value} onChange={onChange} placeholder={placeholder}
      style={{ ...style, overflow: "hidden", resize: "none", scrollbarWidth: "none", msOverflowStyle: "none" }} />
  );
}
...
function ClientSearch({ value, onChange, ink }) {
  ...
  useEffect(() => {
    if (showList) dbLoadClients().then(setAllClients);
  }, [showList]);
  ...
  const handleSearch = (e) => {
    ...
    setQuery(q);
    if (q.length > 0) {
      setResults(allClients.filter(c => c.name.toLowerCase().includes(q.toLowerCase()) || (c.address || "").toLowerCase().includes(q.toLowerCase())));
    } else {
  ...
  const selectClient = (client) => {
    onChange(client.address, client.abn); // `abn` matches the actual column name
    setShowList(false);
    setQuery("");
  };
    ...
    <div ref={wrapRef} style={{ position: "relative" }}>
      <div className="search-bar-row" style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
        ...
        <div style={{ flex: 1, display: "flex", gap: 4 }}>
          <input value={query} onChange={handleSearch} onFocus={() => setShowList(true)}
          ...
          {allClients.length > 0 && (
            <span style={{ fontFamily: "monospace", fontSize: 10, color: "#8888CC", alignSelf: "center", whiteSpace: "nowrap" }}>
      ...
      {showList && results.length > 0 && (
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", border: "1px solid #C8C0A0", borderRadius: "0 0 8px 8px", boxShadow: "0 4px 16px rgba(0,0,0,0.15)", zIndex: 1000, maxHeight: 200, overflowY: "auto" }}>
          {results.map((client, i) => (
            <div key={i} onClick={() => selectClient(client)}
      ...
      {showList && query.length > 0 && results.length === 0 && (
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", border: "1px solid #C8C0A0", borderRadius: "0 0 8px 8px", padding: "12px", fontFamily: "monospace", fontSize: 11, color: "#8888CC", zIndex: 1000 }}>
          No clients found for "{query}"
        </div>
      )}

      <AutoTextarea value={value} onChange={e => onChange(e.target.value, null)}
...
function SigCanvas() {
  ...
  const draw = (e) => {
    ...
    ctx.strokeStyle = "#2D2D7A"; ctx.lineWidth = 1.5; ctx.lineCap = "round"; ctx.stroke();
    last.current = pos;
  };
  ...
  return (
    <div>
      <canvas ref={canvasRef} width={340} height={80}
        style={{ border: "1px dashed #9999CC", borderRadius: 4, background: "rgba(255,255,255,0.6)", cursor: "crosshair", display: "block", touchAction: "none", maxWidth: "100%" }}
...
function Toast({ msg }) {
  if (!msg) return null;
  return (
    <div style={{ position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)", background: "#2D2D7A", color: "#fff", fontFamily: "monospace", fontSize: 13, padding: "10px 20px", borderRadius: 8, zIndex: 9999, pointerEvents: "none", whiteSpace: "nowrap", boxShadow: "0 4px 16px rgba(0,0,0,0.25)" }}>
      {msg}
    </div>
  );
}
...
export default function App({ user }) {
    ...
    async function init() {
      ...
      try {
        ...
        if (profile.coName || profile.coAbn || profile.coAddr || profile.coPhone) {
          setDoc(d => ({
            ...d,
            coName: profile.coName || "",
            coAbn: profile.coAbn || "",
            coAddr: profile.coAddr || "",
            coPhone: profile.coPhone || "",
            from: profile.from || "",
            abnS: profile.abnS || "",
          }));
        }
  ...
  const updateCompany = (field, value) => {
    setDoc(d => {
      ...
      const fromParts = [name, abn ? "ABN: " + abn : "", addr, phone].filter(Boolean);
      updated.from = fromParts.join("\n");
      if (field === "coAbn") updated.abnS = value;
      const profile = loadProfile();
      profile[field] = value;
      profile.from = updated.from;
      profile.abnS = updated.abnS || profile.abnS || "";
  ...
  const updateRow = (idx, field, val) => {
    setDoc(d => {
      const rows = d.rows.map((r, i) => {
        ...
        return (field === "qty" || field === "each") ? calcRow(updated) : updated;
      });
      return { ...d, rows };
    });
  };
  ...
  const newDoc = () => {
    ...
    const profile = loadProfile();
    const fresh = emptyDoc(mode);
    fresh.coName = profile.coName || "";
    fresh.coAbn = profile.coAbn || "";
    fresh.coAddr = profile.coAddr || "";
    fresh.coPhone = profile.coPhone || "";
    fresh.from = profile.from || "";
    fresh.abnS = profile.abnS || "";
    setDoc(fresh);
    setDocId(null);
  ...
  const saveDoc = async () => {
    ...
    try {
      ...
      const docs = await dbLoadDocuments();
      setSaved(docs);
      const prefix = result.prefix || (isQuote ? "QT" : "INV");
      const number = result.number || doc.number;
      showToast(`✓ ${isQuote ? "Quote" : "Invoice"} ${prefix}-${number} saved`);
    } catch (e) {
  ...
  const loadDoc = (item) => {
    const profile = loadProfile();
    setDoc({
      ...emptyDoc(item.docType || "invoice"),
      ...item,
      coName: profile.coName || "",
      coAbn: profile.coAbn || "",
      coAddr: profile.coAddr || "",
      coPhone: profile.coPhone || "",
      from: item.from || profile.from || "",
      rows: item.rows?.length ? item.rows : Array.from({ length: 8 }, emptyRow),
    });
    setModeState(item.docType || "invoice");
    setDocId(item.id);
    setPayStatusState(item.payStatus || "unpaid");
    setDrawerOpen(false);
    showToast("Loaded " + (item.prefix || "INV") + "-" + (item.number || ""));
  };
  ...
  const convertToInvoice = async () => {
    ...
    if (docId) {
      const userId = await getUserId();
      await supabase.from("documents").update({ converted: true }).eq("id", docId).eq("user_id", userId);
    }
    setModeState("invoice");
    setDoc(d => ({ ...d, docType: "invoice", convertedToInvoice: true }));
    setDocId(null);
  ...
  return (
    <div style={S.body}>
      ...
      {loading && (
        <div style={{ width: "100%", background: "#E8F5E9", textAlign: "center", padding: "4px", fontFamily: "monospace", fontSize: 11, color: "#2E7D32", letterSpacing: 1 }}>
          ⏳ Syncing with cloud...
        </div>
      )}
      ...
      <div id="invoice-paper" style={S.paper}>
        <div style={{ fontFamily: "monospace", fontSize: 11, letterSpacing: 1.5, color: isQuote ? "#7AB898" : "#8888CC", textTransform: "uppercase", marginBottom: 6 }}>
          {isQuote ? "Original Copy — Quote" : "Original Copy"}
        </div>
        <div style={{ fontFamily: "Georgia, serif", fontSize: 26, color: ink, textAlign: "center", letterSpacing: 1, border: `2px solid ${ink}`, padding: "8px 16px", marginBottom: 16 }}>
          {isQuote ? "Quote / Estimate" : "Tax Invoice / Statement"}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, gap: 16 }}>
          ...
          <div style={{ flex: 1, textAlign: "right" }}>
            {[["coName", "Your Company Name", 17, 700], ["coAbn", "ABN: XX XXX XXX XXX", 12, 400], ["coAddr", "Address, City, State", 12, 400], ["coPhone", "Phone / Email", 12, 400]].map(([field, ph, fs, fw]) => (
              <input key={field} value={doc[field] || ""} onChange={e => updateCompany(field, e.target.value)} placeholder={ph}
                style={{ display: "block", width: "100%", textAlign: "right", border: "none", background: "transparent", fontFamily: "Lato, sans-serif", color: ink, padding: "2px 4px", outline: "none", fontSize: fs, fontWeight: fw }} />
            ))}
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", border: `1.5px solid ${ink}`, borderTop: "none" }}>
          <div style={{ padding: "8px 10px", minHeight: 80, borderRight: `1.5px solid ${ink}` }}>
            ...
            <AutoTextarea value={doc.from || ""} onChange={e => setDoc(d => ({ ...d, from: e.target.value }))}
          ...
          <div style={{ padding: "8px 10px", minHeight: 80, position: "relative" }}>
            <ClientSearch value={doc.to || ""} ink={ink}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", border: `1.5px solid ${ink}`, borderTop: "none" }}>
          {[["ABN (Supplier):", "abnS", "XX XXX XXX XXX"], ["ABN (Recipient):", "abnR", "XX XXX XXX XXX"]].map(([lbl, field, ph], i) => (
            <div key={lbl} style={{ padding: "5px 10px", borderRight: i === 0 ? `1.5px solid ${ink}` : "none", display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <input value={doc[field] || ""} onChange={e => setDoc(d => ({ ...d, [field]: e.target.value }))} placeholder={ph}
                style={{ border: "none", background: "transparent", fontFamily: "monospace", fontSize: 13, color: ink, flex: 1, outline: "none" }} />
            </div>
          ))}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", border: `1.5px solid ${ink}`, borderTop: "none", padding: "4px 10px", marginBottom: 14 }}>
          ...
          <input value={doc.gstNo || ""} onChange={e => setDoc(d => ({ ...d, gstNo: e.target.value }))} placeholder="Enter GST number"
            style={{ border: "none", background: "transparent", fontFamily: "monospace", fontSize: 13, color: ink, outline: "none", flex: 1, margin: "0 12px" }} />
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", border: `1.5px solid ${ink}` }}>
          <thead>
            <tr>
              {[["QTY", "60px"], ["Description", "auto"], ["Each $", "90px"], ["GST 10%", "80px"], ["Total $", "100px"], ["", "32px"]].map(([h, w]) => (
                <th key={h} style={{ ...S.th, width: w, textAlign: h === "Description" ? "left" : "center" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {doc.rows.map((row, idx) => (
              <tr key={row.id} style={{ background: idx % 2 === 1 ? `rgba(${isQuote ? "180,220,200" : "200,200,232"},0.08)` : "transparent" }}>
                ...
                <td className="remove-row-btn" style={S.td}><button onClick={() => removeRow(idx)} style={{ width: "100%", height: "100%", border: "none", background: "transparent", color: "#C0392B", cursor: "pointer", fontSize: 14 }}>✕</button></td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        {payStatus !== "unpaid" && (
          <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%) rotate(-30deg)", fontFamily: "monospace", fontSize: 60, fontWeight: 700, color: payStatus === "paid" ? "rgba(16,185,129,0.12)" : "rgba(239,68,68,0.12)", pointerEvents: "none", whiteSpace: "nowrap", zIndex: 0 }}>
      ...
      <div className="no-print" style={{ width: "100%", maxWidth: 820, marginTop: 12, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", background: "rgba(0,0,0,0.06)", borderRadius: 8 }}>
        <span style={{ fontFamily: "monospace", fontSize: 12, color: "#4A3F00", fontWeight: 700, letterSpacing: 0.5 }}>
          📄 Terms & Conditions <span style={{ fontWeight: 400, color: "#888", fontSize: 11 }}>(prints on page 2)</span>
        </span>
        <div style={{ display: "flex", gap: 8 }}>
          {tcEditing && (
            <button onClick={() => { saveTC(DEFAULT_TC); setTcText(DEFAULT_TC); }} style={{ padding: "6px 12px", borderRadius: 6, border: "1px solid #C0392B", background: "transparent", color: "#C0392B", fontFamily: "monospace", fontSize: 11, cursor: "pointer" }}>↺ Reset to Default</button>
          )}
          <button onClick={() => setTcEditing(e => !e)} style={{ padding: "6px 14px", borderRadius: 6, border: "none", background: tcEditing ? "#2E7D32" : "#2D2D7A", color: "#fff", fontFamily: "monospace", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
            {tcEditing ? "✓ Done Editing" : "✏️ Edit T&C"}
          </button>
        </div>
      </div>
      ...
      {tcEditing && (
        <div className="no-print" style={{ width: "100%", maxWidth: 820, background: "#fff", border: "1px solid #C8C0A0", borderRadius: 8, padding: 16, marginTop: 4 }}>
          ...
          <textarea value={tcText} onChange={e => { setTcText(e.target.value); saveTC(e.target.value); }}
            style={{ width: "100%", height: 400, fontFamily: "monospace", fontSize: 12, color: "#333", border: "1px solid #ddd", borderRadius: 6, padding: 12, outline: "none", resize: "vertical", lineHeight: 1.7 }} />
        </div>
      )}
      ...
      <div className="tc-page" style={{ width: "100%", maxWidth: 820, background: "#fff", padding: "40px 48px", fontFamily: "Lato, sans-serif", fontSize: 12, color: "#333", lineHeight: 1.7 }}>
        ...
        <div style={{ marginTop: 24, borderTop: "1px solid #ccc", paddingTop: 12, fontSize: 10, color: "#888", textAlign: "center" }}>
          By accepting this invoice, the Buyer agrees to be bound by these Terms and Conditions. &nbsp;|&nbsp; {doc.coName || "Company"} &nbsp;|&nbsp; {doc.coPhone || ""} &nbsp;|&nbsp; {doc.coAddr || ""}
        </div>
      </div>
      ...
      <div className="no-print" style={{ width: "100%", maxWidth: 820, marginTop: 16 }}>
        <button onClick={async () => {
          ...
          if (opening) { setLoading(true); const docs = await dbLoadDocuments(); setSaved(docs); setLoading(false); }
        }}
                  ...
                  <div key={inv.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #eee", flexWrap: "wrap", gap: 8 }}>
                    <div>
                      <div style={{ fontWeight: 700, color: iq ? "#C47A00" : "#C0392B", fontFamily: "monospace" }}>
                        {docNum}
                        <span style={{ display: "inline-block", fontSize: 10, padding: "2px 7px", borderRadius: 4, marginLeft: 6, background: iq ? "#D4EEDC" : "#E0E0F8", color: iq ? "#1A5C3A" : "#2D2D7A", border: `1px solid ${iq ? "#90C8A0" : "#9999CC"}` }}>
                          {inv.convertedToInvoice ? "CONVERTED" : iq ? "QUOTE" : "INVOICE"}
                        </span>
                        <span style={{ display: "inline-block", fontSize: 10, padding: "2px 7px", borderRadius: 4, marginLeft: 4, background: inv.payStatus === "paid" ? "#D1FAE5" : inv.payStatus === "overdue" ? "#FEE2E2" : "#FEF3C7", color: inv.payStatus === "paid" ? "#065F46" : inv.payStatus === "overdue" ? "#991B1B" : "#92400E" }}>
                          {inv.payStatus || "unpaid"}
                        </span>
                      </div>
                      <div style={{ fontFamily: "monospace", fontSize: 11, color: "#8888CC" }}>
                        {inv.date} · {(inv.to || "").split("\n")[0] || "—"} · ${inv.grandTotal || "0.00"}
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: 6 }}>
                      {iq && !inv.convertedToInvoice && <button onClick={() => { loadDoc(inv); setTimeout(convertToInvoice, 200); }} style={S.btn("#1A5C3A")}>⚡</button>}
                      <button onClick={() => loadDoc(inv)} style={S.btn("#2D2D7A")}>Load</button>
                      <button onClick={() => deleteDoc(inv.id, docNum)} style={S.btn("#C0392B")}>✕</button>
                    </div>
                  </div>
                );
              })
