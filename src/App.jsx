...
const COUNTER_KEY = "invoice_app_counters";
...
function saveClient(toAddress, abnR) {
  ...
  const entry = { name, address: toAddress, abnR: abnR || "", updatedAt: new Date().toISOString() };
  if (existing !== -1) clients[existing] = entry;
  else clients.unshift(entry);
  // Keep max 100 clients
  try { localStorage.setItem(CLIENTS_KEY, JSON.stringify(clients.slice(0, 100))); } catch {}
}
...
function getNextNumber(type) {
  try {
    const counters = JSON.parse(localStorage.getItem(COUNTER_KEY) || '{"invoice":0,"quote":0}');
    counters[type] = (counters[type] || 0) + 1;
    localStorage.setItem(COUNTER_KEY, JSON.stringify(counters));
    return String(counters[type]).padStart(6, "0");
  } catch { return "000001"; }
}
...
// ── FIXED GST: qty=1, price=100 → GST=10.00, Total=110.00
...
function ClientSearch({ value, onChange, ink }) {
  ...
  const handleSearch = (e) => {
    ...
    setQuery(q);
    if (q.length > 0) {
      const filtered = allClients.filter(c => c.name.toLowerCase().includes(q.toLowerCase()) || c.address.toLowerCase().includes(q.toLowerCase()));
      setResults(filtered);
      setShowList(true);
    } else {
  ...
  const selectClient = (client) => {
    onChange(client.address, client.abnR);
    setShowList(false);
    setQuery("");
  };
      ...
      <div className="search-bar-row" style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
        ...
        <div style={{ flex: 1, display: "flex", gap: 4 }}>
          <input
            ...
            onFocus={() => { setResults(allClients); setShowList(true); }}
        ...
        <div style={{ position: "absolute", top: "100%", left: 0, right: 0, background: "#fff", border: "1px solid #C8C0A0", borderRadius: "0 0 8px 8px", boxShadow: "0 4px 16px rgba(0,0,0,0.15)", zIndex: 1000, maxHeight: 200, overflowY: "auto" }}>
          {results.map((client, i) => (
            <div key={i} onClick={() => selectClient(client)}
              style={{ padding: "8px 12px", cursor: "pointer", borderBottom: "1px solid #f0f0f0", transition: "background 0.1s" }}
              ...
              {client.abnR && <div style={{ fontFamily: "monospace", fontSize: 10, color: "#8888CC" }}>ABN: {client.abnR}</div>}
            </div>
          ))}
        </div>
      )}
...
export default function App() {
  ...
  // Load saved profile and invoices on startup
  useEffect(() => {
    setSaved(load());
  ...
  // Also saves company profile to localStorage permanently
  ...
  const newDoc = () => {
    ...
    // Restore company profile into new doc
  ...
  const saveDoc = () => {
    const all = load();
    const data = { ...doc, payStatus, savedAt: new Date().toISOString() };
    if (docId) {
      ...
      data.number = num; data.prefix = prefix;
      data.id = "local_" + Date.now();
      setDocId(data.id);
      setDoc(d => ({ ...d, number: num, prefix }));
      all.unshift(data);
    }
    save(all); setSaved(load());
    // Save client to directory automatically
    saveClient(doc.to, doc.abnR);
    const prefix = isQuote ? "QT" : "INV";
    showToast(`✓ ${isQuote ? "Quote" : "Invoice"} ${prefix}-${data.number || doc.number} saved`);
  };

  const loadDoc = (item) => {
    setDoc({ ...emptyDoc(item.docType || "invoice"), ...item, rows: item.rows?.length ? item.rows : Array.from({ length: 8 }, emptyRow) });
  ...
  const deleteDoc = (id, label) => {
    if (!window.confirm("Delete " + label + "?")) return;
    const all = load().filter(d => d.id !== id);
    save(all); setSaved(all);
  ...
  const convertToInvoice = () => {
    ...
    if (docId) {
      const idx = all.findIndex(d => d.id === docId);
      if (idx !== -1) { all[idx].convertedToInvoice = true; save(all); }
    }
    setModeState("invoice");
    setDoc(d => ({ ...d, docType: "invoice" }));
    setDocId(null);
  ...
  const handleLogoUpload = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      // Save logo permanently to profile
      const profile = loadProfile();
  ...
  return (
    <div style={S.body}>
      <style>{`
        ...
        @media print {
          * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
          ...
          /* Hide everything except invoice and T&C */
          ...
          #invoice-paper {
            ...
            border-left: 6px solid #B8A870 !important;
            margin: 0 !important;
      ...
      {/* ── Terms & Conditions Page (prints as page 2) ── */}
      <style>{`
        @media print {
          .tc-page { display: block !important; page-break-before: always !important; }
        }
        .tc-page { display: none; }
      `}</style>

      {/* T&C Edit Button - screen only */}
      <div className="no-print" style={{ width: "100%", maxWidth: 820, marginTop: 12, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", background: "rgba(0,0,0,0.06)", borderRadius: 8 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {tcEditing && (
            <button onClick={() => { saveTC(DEFAULT_TC); setTcText(DEFAULT_TC); }} style={{ padding: "6px 12px", borderRadius: 6, border: "1px solid #C0392B", background: "transparent", color: "#C0392B", fontFamily: "monospace", fontSize: 11, cursor: "pointer" }}>
              ↺ Reset to Default
            </button>
          )}
      ...
      {tcEditing && (
        <div className="no-print" style={{ width: "100%", maxWidth: 820, background: "#fff", border: "1px solid #C8C0A0", borderRadius: 8, padding: 16, marginTop: 4 }}>
          <div style={{ fontFamily: "monospace", fontSize: 11, color: "#888", marginBottom: 8 }}>
            Edit your terms and conditions below. Changes save automatically. Use plain text — each numbered clause on its own line.
          </div>
          <textarea
            value={tcText}
            onChange={e => { setTcText(e.target.value); saveTC(e.target.value); }}
      ...
      <div className="tc-page" style={{ width: "100%", maxWidth: 820, background: "#fff", padding: "40px 48px", fontFamily: "Lato, sans-serif", fontSize: 12, color: "#333", lineHeight: 1.7 }}>
        ...
        <div style={{ whiteSpace: "pre-wrap", fontSize: 11, color: "#444", lineHeight: 1.8 }}>
          {tcText}
        </div>
      ...
      <div className="no-print" style={{ width: "100%", maxWidth: 820, marginTop: 16 }}>
        <button onClick={() => { setDrawerOpen(o => !o); if (!drawerOpen) setSaved(load()); }}
